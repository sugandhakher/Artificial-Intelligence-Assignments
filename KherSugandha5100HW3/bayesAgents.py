...
def constructBayesNet(gameState):
    ...
    for observation in obsVars:
        edges.append((FOOD_HOUSE_VAR, observation))
        edges.append((GHOST_HOUSE_VAR, observation))
        variableDomainsDict[observation] = OBS_VALS
...
def fillObsCPT(bayesNet, gameState):
    ...
    houseLocations = {bottomLeftPos: BOTTOM_LEFT_VAL, topLeftPos: TOP_LEFT_VAL,
                 bottomRightPos: BOTTOM_RIGHT_VAL, topRightPos: TOP_RIGHT_VAL}

    domainDict = bayesNet.variableDomainsDict()

    for housePosition in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePosition):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsVarFactor = bn.Factor([obsVar], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], domainDict)


            for assignment in obsVarFactor.getAllPossibleAssignmentDicts():
                houseLoc = houseLocations[housePosition]

                if assignment[FOOD_HOUSE_VAR] == houseLoc:
                    probabilityRed = PROB_FOOD_RED
                    probabilityBlue = 1 - PROB_FOOD_RED
                    probabilityNone = 0

                elif assignment[GHOST_HOUSE_VAR] == houseLoc:
                ...
                if assignment[obsVar] == BLUE_OBS_VAL:
                    obsVarFactor.setProbability(assignment, probabilityBlue)
                elif assignment[obsVar] == RED_OBS_VAL:
                    obsVarFactor.setProbability(assignment, probabilityRed)
                else:
                    obsVarFactor.setProbability(assignment, probabilityNone)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    factor = inferenceByVariableElimination(bayesNet, HOUSE_VARS, evidence, eliminationOrder)

    maximumProbability = 0.0
    bestProbability = 0.0
...
class VPIAgent(BayesAgent):
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for probability, newEvidence in exploration:
            computeValue = self.computeEnterValues(newEvidence, enterEliminationOrder)
            expectedValue += probability * max(computeValue)
