...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        probabilityTables = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        for variable in eliminationOrder:
            probabilityTables, combinedFactor = joinFactorsByVariable(probabilityTables, variable)

            if len(combinedFactor.unconditionedVariables()) is not 1:
                probabilityTables.append(eliminate(combinedFactor, variable))

        return normalize(joinFactors(probabilityTables))
