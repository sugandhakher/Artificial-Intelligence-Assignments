...
def joinFactors(factors):
    ...
    joinedUnconditionedVariables = []

    for factor in factors:
        for conditionedVariable in factor.conditionedVariables():
            if conditionedVariable not in joinedConditionedVariables:
                joinedConditionedVariables.append(conditionedVariable)

        for unconditionedVariable in factor.unconditionedVariables():
            if unconditionedVariable not in joinedUnconditionedVariables:
                joinedUnconditionedVariables.append(unconditionedVariable)

    for unconditionedVariable in joinedUnconditionedVariables:
        if unconditionedVariable in joinedConditionedVariables:
            joinedConditionedVariables.remove(unconditionedVariable)

    domain = factors[0].variableDomainsDict()

    newFactor = Factor(joinedUnconditionedVariables, joinedConditionedVariables, domain)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        for variable in factor.unconditionedVariables():
            if variable not in eliminationVariable:
                unconditioned.append(variable)

        domain = factor.variableDomainsDict()

        newFactor = Factor(unconditioned, factor.conditionedVariables(), domain)
